...
object Benchmark2 {
    ...
    private val sp = AppContext.context.getSharedPreferences("sp_2", Context.MODE_PRIVATE)
    private val dataStore = PreferenceDataStoreFactory.create {
        File(
            PathManager.filesDir,
            "data_store_2.preferences_pb"
        )
    }
    private val sqliteKV = SQLiteKV.Builder("default_sqlite").build()
    private val fastkv = FastKV.Builder(PathManager.fastKVDir, "fastkv_2").blocking().build()
    ...
    private suspend fun runTest(context: Context) {
        ...
        Log.i(
            TAG, "Fill, sp: " + time[0] / MILLION + ", dataStore: " + time[1] / MILLION
                    + ", sqlite: " + time[2] / MILLION + ", fastkv:" + time[3] / MILLION
        )
        Arrays.fill(time, 0L)
        ...
        for (i in 0 until round) {
            ...
            Log.d(
                TAG, "Update, sp: " + a / MILLION + ", dataStore: " + b / MILLION
                        + ", sqlite: " + c / MILLION + ", fastkv:" + d / MILLION
            )
        }
        Log.i(
