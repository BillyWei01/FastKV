...
class FastBuffer {
  ...
  public short getShort() {
    return (short) (((hb[position++] & 0xFF)) | ((hb[position++]) << 8));
  }
  ...
  public String getString(FastCipher cipher, int len) {
    ...
    if (dst == null) {
      ...
      str =
          dst.length > MAX_CHAR_LEN
              ? new String(dst, StandardCharsets.UTF_8)
              : decodeStr(dst, 0, dst.length);
    }
    position += len;
    return str;
  }
  ...
  public String getString(int len) {
    if (len < 0) {
      ...
      String str =
          len > MAX_CHAR_LEN
              ? new String(hb, position, len, StandardCharsets.UTF_8)
              : decodeStr(hb, position, len);
      position += len;
      return str;
    }
  }
