 ...
 * async-block: write all data to disk with blocking I/O asynchronously, likes 'apply' of
 * SharePreferences.<br>
 ...
 * 2. Do not change cipher , or you will lost data. But it's okay to transfer from no cipher to
 * applying cipher.<br>
...
public final class FastKV extends AbsFastKV {
  ...
  // If mmap API throw IOException, degrade to blocking mode (write all data to disk with blocking
  // I/O).
  ...
  private void loadFromABFile() {
    ...
    try {
      ...
      try {
        aBuffer =
            aChannel.map(FileChannel.MapMode.READ_WRITE, 0, aFileLen > 0 ? aFileLen : PAGE_SIZE);
        aBuffer.order(ByteOrder.LITTLE_ENDIAN);
        bBuffer =
            bChannel.map(FileChannel.MapMode.READ_WRITE, 0, bFileLen > 0 ? bFileLen : PAGE_SIZE);
      ...
      if (aFileLen == 0 && bFileLen == 0) {
        ...
        if (aDataSize >= 0 && (aDataSize <= aFileLen - DATA_START)) {
          ...
          if (aCheckSum == fastBuffer.getChecksum(DATA_START, aDataSize)
              && parseData(aHadEncrypted)) {
            checksum = aCheckSum;
            isAValid = true;
          }
        }
        if (isAValid) {
          ...
          if (bDataSize >= 0 && (bDataSize <= bFileLen - DATA_START)) {
            ...
            if (bCheckSum == fastBuffer.getChecksum(DATA_START, bDataSize)
                && parseData(bHadEncrypted)) {
      ...
      if (oldFileName != null) {
        if (writingMode == NON_BLOCKING) {
          FastKVConfig.getExecutor()
              .execute(() -> Utils.deleteFile(new File(path + name, oldFileName)));
        } else {
  ...
  /**
   * Batch put objects. Only support type in [boolean, int, long, float, double, String, byte[], Set
   * of String] and object with encoder.
   *
  ...
  /**
   * Forces any changes to be written to the storage device containing the mapped file. No need to
   * call this unless what's had written is very import. The system crash or power off before data
   * syncing to disk might make recently update lost.
   */
  ...
  /**
   * When you open file with mode of SYNC_BLOCKING or ASYNC_BLOCKING, It will auto commit after
   * every putting or removing, by default. If you need to batch update several key-values, you
   * could call this method at first, and call {@link #commit()} after updating, that method will
   * recover {@link #autoCommit} to 'true' again.
   */
  ...
  protected void checkGC() {
    if (invalidBytes >= (bytesThreshold() << 1)
        || invalids.size()
            >= (dataEnd < (1 << 14) ? BASE_GC_KEYS_THRESHOLD : BASE_GC_KEYS_THRESHOLD << 1)) {
      gc(0);
    }
  }
  ...
  public static final class Builder {
     ...
     *
     * <p>In non-blocking mode (write data with mmap), it might lost update if the system crash or
     * power off before flush data to disk. You could use {@link #force()} to avoid losing update,
     * or use SYNC_BLOCKING mode.
     *
     * <p>In blocking mode, every update will write all data to the file, which is expensive cost.
     *
     * <p>So it's recommended to use blocking mode only if the data is every important.
     *
     * <p>
     *
     * @return the builder
  ...
  /**
   * Adapt old SharePreferences, return a new SharedPreferences with storage strategy of FastKV.
   *
   * <p>Node: The old SharePreferences must implement getAll() method, otherwise can not import old
   * data to new files.
   *
