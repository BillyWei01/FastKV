...
/**
 * struct of primary type (boolean/int/float/long/double): [type|keyLen|keyData|value]
 *
 * <p>struct of variable type (string/array/object): [type|keyLen|keyData|valueLen|value]
 *
 * <p>type: 1 byte, keyLen: 1 byte, valueLen: 2 bytes.
 */
class Container {
  abstract static class BaseContainer {
  ...
  static class BooleanContainer extends BaseContainer {
    ...
    boolean equalTo(BaseContainer other) {
      return other.getType() == DataType.BOOLEAN && ((BooleanContainer) other).value == this.value;
    }
  }
  ...
  static class IntContainer extends BaseContainer {
    ...
    @Override
    boolean equalTo(BaseContainer other) {
      return other.getType() == DataType.INT && ((IntContainer) other).value == this.value;
    }
  }
  ...
  static class FloatContainer extends BaseContainer {
    ...
    @Override
    boolean equalTo(BaseContainer other) {
      return other.getType() == DataType.FLOAT && ((FloatContainer) other).value == this.value;
    }
  }
  ...
  static class LongContainer extends BaseContainer {
    ...
    @Override
    boolean equalTo(BaseContainer other) {
      return other.getType() == DataType.LONG && ((LongContainer) other).value == this.value;
    }
  }
  ...
  static class DoubleContainer extends BaseContainer {
    ...
    @Override
    boolean equalTo(BaseContainer other) {
      return other.getType() == DataType.DOUBLE && ((DoubleContainer) other).value == this.value;
    }
  }

  abstract static class VarContainer extends BaseContainer {
    Object value;
    int start;
    int valueSize;
    boolean external;
